      ...
      m_coreProcess(coreProcess)
{
  ...
  for (const auto &languageCode : I18N::supportedLanguageCodes()) {
    const auto nativeName = I18N::toNativeName(languageCode);
    if (!nativeName.isEmpty())
      ui->comboLanguage->addItem(nativeName, languageCode);
  }
  ui->comboLanguage->setCurrentIndex(ui->comboLanguage->findData(I18N::currentLanguage()));
