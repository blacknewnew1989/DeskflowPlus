...
### 0.2 当前产品变更：紧凑界面、权限门控、临时品牌、托盘与多语言
...
多语言是本轮 P0 交付的一部分，不得只保留英文/简体中文占位：

- RelayDesk 唯一语言清单为英语（`en`）、西班牙语（`es`）、意大利语（`it`）、日语（`ja`）、
  韩语（`ko`）、俄语（`ru`）和简体中文（`zh_CN`）；Windows、macOS、共享 Qt、翻译脚本与
  打包流程必须消费同一清单，不得由平台代理分别维护；
- 所有 RelayDesk 新界面中的用户可见文本必须进入翻译目录，覆盖首页、权限、设备、发现、
  配对、信任、传输、历史、托盘、设置、错误与空状态；不得以源码硬编码或英文 fallback
  冒充已完成翻译；
- 设置中提供语言选择器。选择值持久化，重新打开应用后保持；切换后当前界面必须生效，若
  个别上游窗口只能重建后刷新，界面需明确提示并在下一次窗口创建时使用新语言；
- Windows 与 macOS 包必须携带七种 RelayDesk `.qm`，并自动测试语言发现、切换、持久化、
  目录键完整性和安装包资源闭包。
...
| FR-UI-003 | 配对向导 | P0 |
| FR-UI-004 | 传输中心 | P0 |
| FR-UI-005 | 完整支持英语、西班牙语、意大利语、日语、韩语、俄语、简体中文；所有 RelayDesk 用户可见文本使用统一翻译目录 | P0 |
| FR-UI-006 | 托盘快速操作 | P0 |
...
| FR-UI-012 | 设置提供由统一语言清单驱动的语言选择器，选择后持久化并应用到界面 | P0 |
| FR-UI-013 | 首页、权限、设备、配对、传输、历史、托盘、设置、错误与空状态的七语言 key 集合完整一致，不允许未完成占位 | P0 |
| FR-UI-014 | Windows/macOS 安装包均携带七种 RelayDesk `.qm`，并由打包测试验证资源存在和可加载 | P0 |
| FR-UI-015 | 自动测试语言发现、运行时切换、重启持久化、未知语言回退以及跨平台统一语言清单 | P0 |
...
- 七语言清单只有一个代码/构建单源；翻译目录、设置选择器、运行时加载器和两个平台的打包
  清单不得复制维护不同集合；
- CI 必须拒绝 RelayDesk catalog 的缺失 key、额外 key、重复 key、unfinished 条目，以及安装包
  缺少任一受支持语言 `.qm`。
...
10. 七种 RelayDesk `.qm` 及语言发现/切换/持久化/打包资源测试证据；
11. 已知问题与 `NOT_RUN`。
...
在 Windows 与 macOS 安装包中分别验证英语、西班牙语、意大利语、日语、韩语、俄语和简体
中文均可被设置语言选择器发现。逐个切换语言后，首页、权限、设备、配对、传输、历史、托盘、
设置、错误与空状态使用对应 catalog；重启应用后选择保持。未知或损坏语言值安全回退到英语，
不显示空白 key。自动检查七份 RelayDesk catalog key 集完全一致、无重复和 unfinished 条目，且
最终 App/DMG/MSI/便携包都包含并能加载七种 `.qm`。
