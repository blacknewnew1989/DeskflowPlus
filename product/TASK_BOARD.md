...
|---|---|---|---|
| UI-010 | 按已确认设计图实现共享 Qt 紧凑单栏首页 | A3/A0 | `d419b56f2`；默认 560×420、最小 520×380，紧凑首页/权限详情与针对性 Qt 回归 PASS；待精确标签双平台构建 |
| BRAND-002 | 实现原创“双设备 + 中继点”临时 Logo | A3/A4/A5 | `23ac2cf4e`；SVG 单源、5 份主题资源、ICO/ICNS/CMake 接线和 16 px 连通性校验 PASS；待平台包核验 |
| TRAY-001 | 完成最小化/关闭到托盘与后台生命周期 | A3/A4/A5 | `ce3f81d7f`、`d0d449edc`、`d419b56f2`；独立设置与同步安全停机已实现，本地回归 PASS；待双平台编译和 macOS 真机交互 |
| MAC-037 | 适配紧凑首页、权限能力门控和 macOS menu bar | A5/A3/A0 | 首次协调通知已发送；复用共享 Qt/PermissionSnapshot，待三项权限独立门控、激活复检、template 图标和真机 lifecycle 回归 |

以上任务均为 `IN_PROGRESS`。设计图于 2026-08-14 经用户确认；共享实现与本地回归已经
完成，但在精确标签构建、平台产物及 macOS owner 证据齐备前不得移入 Done。详见
