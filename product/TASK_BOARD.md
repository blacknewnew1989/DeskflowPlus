...
|---|---|---|---|
| UI-010 | 按已确认设计图实现共享 Qt 紧凑单栏首页 | A3/A0 | 560×420 紧凑单栏、权限摘要、设备语义和显式“暂停/继续共享”入口已实现；受影响本机 Qt 测试 PASS，待集成与双平台 CI |
| BRAND-002 | 实现原创“双设备 + 中继点”临时 Logo | A3/A4/A5 | SVG 单源、彩色 App/安装包与单色 menu bar 资源已接线；本机品牌和打包契约检查 PASS，待集成与双平台 CI |
| TRAY-001 | 完成最小化/关闭到托盘与后台生命周期 | A3/A4/A5 | 最小化/关闭到后台、一次性提示、显示/暂停共享/退出和真正退出清理已实现；本机 lifecycle 回归 PASS，待集成与双平台 CI |
| MAC-037 | 适配紧凑首页、权限能力门控和 macOS menu bar | A5/A3/A0 | Accessibility 能力门控、紧凑首页、menu bar、窗口恢复和 lifecycle 代码及受影响本机回归已完成；最终直接 Mach-O 测试包授权后的真实 `deskflow-core`/输入链路与双机键鼠为 `NOT_RUN` |
| I18N-002 | 提供七种可选语言和运行时切换 | A5/A3/A0 | 英语、简中、西语、意语、日语、韩语、俄语可运行时立即切换并持久化；App 内 21 个 `.qm` 和翻译/运行时本机测试 PASS，待集成与双平台 CI |

以上任务均为 `IN_PROGRESS`。代码与受影响本机自动化回归已经完成；在改版提交集成、
双平台 CI 和对应平台产物证据齐备前不得移入 Done。当前唯一真机 `NOT_RUN` 边界是最终
稳定、直接 Mach-O 测试包重新授权后的真实 `deskflow-core` 输入链路和 Win↔Mac 双机键鼠，
不得用旧 Phase 4 artifact 或旧 wrapper 测试包代替。
