...
class WindowsInstallRegressionScriptTests(unittest.TestCase):
    ...
    def test_checks_service_firewall_and_real_user_data_boundary(self) -> None:
        ...
        self.assertIn('$ExpectedPrograms = @{', SCRIPT)
        self.assertIn('"$ProductName Server" = Join-Path $InstallRoot "deskflow-core.exe"', SCRIPT)
        self.assertIn('"$ProductName Client" = Join-Path $InstallRoot "deskflow.exe"', SCRIPT)
